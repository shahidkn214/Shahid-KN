...
import { History, Trash2, Video, Headphones, Clock, ShieldCheck, AlertTriangle, X } from 'lucide-react';
    ...
    <div id="download-history-section" className="w-full rounded-2xl bg-gray-900/60 backdrop-blur-md border border-gray-800/80 p-5 sm:p-6 space-y-4 shadow-xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-gray-800/60">
            ...
            <p className="text-[11px] text-gray-400">
              {currentUser ? (
                <span className="text-emerald-400 font-medium flex items-center gap-1">
                  <ShieldCheck className="w-3 h-3 inline" /> Persistent Cloud Database (Soft-delete protected)
                </span>
              ) : (
                <span className="text-amber-400 font-medium">
                  Guest Session (In-memory only • Disappears on reload)
                </span>
              )}
            </p>
          </div>
        ...
        <button
          ...
          className="self-start sm:self-auto flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-950/60 hover:bg-red-950/40 text-gray-400 hover:text-red-300 border border-gray-800 hover:border-red-500/30 text-xs font-semibold transition"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {items.map((item) => (
          ...
          >
            ...
            {/* Individual Item Soft Delete Button */}
            <button
              type="button"
              onClick={(e) => {
                e.stopPropagation();
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {currentUser
                    ...
                    : 'This will reset your temporary session download list.'}
                </p>
              </div>
            </div>
